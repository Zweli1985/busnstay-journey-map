...
\section{20-Year Halso Gas P\&L – Summary Table}

\begin{longtable}{lrrr}
\toprule
Year & Revenue per home & Total Revenue & Post-tax Profit \\
\midrule
2026 & 240 & 21.6M & 4.86M \\
2027 & 249.6 & 22.5M & 5.06M \\
2030 & 280.8 & 25.3M & 5.71M \\
2035 & 342 & 30.8M & 6.93M \\
2045 & 506 & 45.6M & 10.3M \\
\midrule
\textbf{20-year total} & -- & \textbf{~460M} & \textbf{~103.5M} \\
\bottomrule
\end{longtable}

(Full year-by-year table available in appendix / Excel model)
...
\begin{enumerate}
\item Halso to provide proof of funds (PoF) for R5,000,000 or bank comfort letter
\item RGS legal confirms PoF and schedules MoU / definitive agreements meeting
\item Parties sign MoU and contracts, Halso deposits funds into Fidelity escrow (HASS Law \& Associates)
\item RGS registers Halso's 20\% equity with corporate registrar
\item Upon registration confirmation escrow funds released to RGS account
\item Parallel: finalise gas commercial terms (pricing, refill cadence, installation responsibilities)
\end{enumerate}
\end{rgsbox}
...
Loan parameters (assumed for the illustrative model): principal USD 250,000,000; interest 6% p.a.; term 15 years; annual level payments. The precise level payment and amortisation schedule are generated by the financial model and written to \texttt{financial\_model/loan\_schedule.csv}.
If you re-run the Python model with different \texttt{params.json} settings, the charts will update automatically on the next compile (ensure \texttt{financial\_model/*.csv} are present relative to your LaTeX working directory).
Below we include three charts generated by \texttt{pgfplots} that read the CSV outputs produced by the model: annual housing revenue, loan closing balance over time, and Halso gas post-tax profit by year. If you re-run the Python model with different \texttt{params.json} settings, the charts will update automatically on the next compile (ensure \texttt{financial\_model/*.csv} are present relative to your LaTeX working directory).

\begin{figure}[ht]
  \centering
  % Housing revenue
  \begin{tikzpicture}
    \begin{axis}[width=0.9\textwidth, height=6cm, grid=major, title={Annual housing revenue (USD)}, xlabel=Year, ylabel=Revenue (USD), xticklabel style={rotate=45,anchor=east}]
  \addplot[blue, thick] table[x=year,y=revenue,col sep=comma] {\detokenize{c:/Users/zwexm/LPSN/RGS/financial_model/housing_years.csv}};
      \addlegendentry{Housing revenue}
    \end{axis}
  \end{tikzpicture}
  \caption{Modelled annual housing revenue (from \texttt{financial\_model/housing\_years.csv}).}
\end{figure}
...
\begin{figure}[ht]
  ...
  % Loan balance chart
  \begin{tikzpicture}
    \begin{axis}[width=0.9\textwidth, height=6cm, grid=major, title={BCG loan closing balance (USD)}, xlabel=Year, ylabel=Outstanding balance (USD), ybar=false]
  \addplot[red, very thick] table[x=year,y=closing_balance,col sep=comma] {\detokenize{c:/Users/zwexm/LPSN/RGS/financial_model/loan_schedule.csv}};
      \addlegendentry{Loan closing balance}
    \end{axis}
  \end{tikzpicture}
  \caption{Loan amortisation schedule (closing balance) from \texttt{financial\_model/loan\_schedule.csv}.}
\end{figure}
...
\begin{figure}[ht]
  ...
  % Gas post-tax chart
  \begin{tikzpicture}
    \begin{axis}[width=0.9\textwidth, height=6cm, grid=major, title={Halso gas post-tax profit by year (USD)}, xlabel=Year, ylabel=Post-tax profit (USD)]
  \addplot[green!60!black, thick] table[x=year,y=post_tax,col sep=comma] {\detokenize{c:/Users/zwexm/LPSN/RGS/financial_model/gas_years.csv}};
      \addlegendentry{Gas post-tax profit}
    \end{axis}
  \end{tikzpicture}
  \caption{Annual Halso gas business post-tax profit (from \texttt{financial\_model/gas\_years.csv}).}
\end{figure}
