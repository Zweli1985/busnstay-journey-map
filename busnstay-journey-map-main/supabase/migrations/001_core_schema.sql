...
CREATE TABLE IF NOT EXISTS journeys (
  ...
  CONSTRAINT active_journey_per_passenger UNIQUE(passenger_id, status) WHERE status = 'ACTIVE'
);
...
CREATE TABLE IF NOT EXISTS offline_queue (
  ...
  CONSTRAINT unique_pending_queue UNIQUE(device_id, sequence_number) WHERE processed = false
);
...
CREATE INDEX idx_location_recent ON location_history(journey_id) WHERE created_at > NOW() - INTERVAL '1 hour';
